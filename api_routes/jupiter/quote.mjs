import { apiError, fetchJupiter, isJupiterConfigured, isValidAmount, isValidSlippageBps, json, rateLimit, readUpstream } from '../../api/_lib/roninBackend.mjs'
import { isValidMintAddress } from '../../api/_lib/solanaValidation.mjs'

export default async function handler(req, res) {
  if (req.method !== 'GET') return apiError(res, 405, 'METHOD_NOT_ALLOWED', 'Method not allowed.')
  if (!rateLimit(req, 'solana-quote', 60)) return apiError(res, 429, 'RATE_LIMITED', 'Too many quote requests. Try again shortly.')
  const { inputMint, outputMint, amount, slippageBps, chainId, provider } = req.query || {}
  if (chainId != null && String(chainId) !== '101') return apiError(res, 400, 'INVALID_CHAIN', 'Solana quotes require the Solana chain identifier.')

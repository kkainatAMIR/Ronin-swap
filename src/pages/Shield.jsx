...
export default function Shield() {
  ...
  const security = scanResult?.security || { knownCount: 0, cautionCount: 0, tokenRisks: [], suspicious: [], summary: 'No known issues detected', score: null, scoreLabel: 'Unavailable' }
  ...
  const safeSecurity = security || { knownCount: 0, cautionCount: 0, tokenRisks: [], suspicious: [], summary: 'No known issues detected', score: null, scoreLabel: 'Unavailable' }
  ...
  return (
    <>
      ...
      {scanState === 'ready' && scanResult && <>
        ...
        <section className="section shield-analysis-section enhanced-section"><div className="enhanced-bg" style={{ backgroundImage: "url('/images/hero-ronin.jpg')" }} /><div className="enhanced-ink">分析</div><div className="container" style={{ position: 'relative', zIndex: 1 }}><div className="section-row"><SectionHeading eyebrow="Security analysis" title="Known / suspicious interactions." text="Conservative analysis based only on actual wallet data. No 100% SAFE claims. Uses 🟢 KNOWN, 🟡 CAUTION, 🔴 HIGH RISK only where justified." /><Tag tone={safeSecurity?.suspicious?.length ? 'red' : 'green'}>{safeSecurity?.summary || 'No known issues detected'}</Tag></div><div className="shield-analysis-grid"><div className="surface-card enhanced-card"><div className="panel-heading"><div><Eyebrow>Token risk breakdown</Eyebrow><h3>{safeSecurity.knownCount} known, {safeSecurity.cautionCount} caution</h3></div><Icon name="shield" size={16} /></div><div className="shield-risk-list">{safeSecurity.tokenRisks.slice(0, 20).map((tr, i) => <div key={i} className="shield-risk-item"><span>{tr.symbol || tr.name || shortSig(tr.mint)}</span><small>{tr.mint.slice(0, 4)}...{tr.mint.slice(-4)} • {tr.uiAmountString || tr.uiAmount}</small><Tag tone={tr.risk.level === 'KNOWN' ? 'green' : 'neutral'}>{tr.risk.label}</Tag></div>)}{safeSecurity.tokenRisks.length === 0 && <p className="muted-caption">No token accounts to analyze.</p>}</div></div><div className="surface-card enhanced-card"><div className="panel-heading"><div><Eyebrow>Detected signals</Eyebrow><h3>{safeSecurity.suspicious.length ? `${safeSecurity.suspicious.length} signals` : 'No known issues detected'}</h3></div><Icon name="search" size={16} /></div>{safeSecurity.suspicious.length ? <ul className="shield-suspicious-list">{safeSecurity.suspicious.map((s, i) => <li key={i}><span className="suspicious-icon">{s.type === 'unknown_tokens' ? '🟡' : s.type === 'failed_txs' ? '🔴' : '⚠️'}</span><span>{s.message}</span></li>)}</ul> : <div className="shield-no-threat"><span className="shield-no-threat-icon">🟢</span><strong>No known issues detected</strong><p>Based on current scan, no suspicious token accounts or failed suspicious transactions were found. Always verify before signing.</p></div>}<div className="shield-analysis-foot"><small>Analysis is conservative and based on verifiable on-chain data only. Unable to determine risk where data is insufficient.</small></div></div></div></div></section>

...
export default async function handler(req, res) {
  ...
  try {
    ...
    try {
      const recent = await fetch(`${process.env.SUPABASE_URL}/rest/v1/swap_transactions?wallet_address=eq.${encodeURIComponent(swap.wallet_address)}&verification_status=eq.verified&select=signature,input_mint,output_mint,input_amount_raw,timestamp&order=timestamp.desc&limit=8`, { headers: { apikey: process.env.SUPABASE_SERVICE_ROLE_KEY, Authorization: `Bearer ${process.env.SUPABASE_SERVICE_ROLE_KEY}` } }).then((response) => response.ok ? response.json() : [])
